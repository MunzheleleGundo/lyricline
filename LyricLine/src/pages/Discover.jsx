...
function SectionTitle({ icon: Icon, children }) {
  return (
    ...
    >
      <Icon size={18} color={COLORS.gold} /> {children}
    </h2>
  );
}
  ...
  return (
    <div style={{ minHeight: "100vh", background: COLORS.ink, padding: "clamp(24px, 5vw, 40px) 20px" }}>
      <div style={{ maxWidth: 900, margin: "0 auto" }}>
        ...
        <p style={{ color: COLORS.plum, fontFamily: TYPE.body, fontSize: 14, margin: "0 0 36px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 12 }}>
            {TRENDING_ARTISTS.map((a) => (
              <div key={a.name} style={{ ...cardStyle, padding: 16 }}>
                <div style={{ fontFamily: TYPE.display, fontWeight: 600, fontSize: 15, color: COLORS.cream }}>{a.name}</div>
                <div style={{ display: "flex", justifyContent: "space-between", marginTop: 6 }}>
                  <span style={{ fontFamily: TYPE.body, fontSize: 12, color: COLORS.plum }}>{a.genre}</span>
            ...
            {GLOBAL_CHART.map((t, i) => (
              <div
                ...
                style={{
                  ...
                  borderBottom: i < GLOBAL_CHART.length - 1 ? `1px solid ${COLORS.line}` : "none",
                }}
              >
                <div style={{ fontFamily: TYPE.display, fontSize: 18, color: COLORS.gold, width: 24 }}>{t.rank}</div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontFamily: TYPE.display, fontSize: 15, color: COLORS.cream }}>{t.title}</div>
                  <div style={{ fontFamily: TYPE.body, fontSize: 12, color: COLORS.plum }}>{t.artist}</div>
                </div>
              </div>
            ))}
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {VIRAL_LINES.map((v, i) => (
              <div key={i} style={{ ...cardStyle, padding: 16 }}>
                <div style={{ fontFamily: TYPE.display, fontStyle: "italic", fontSize: 15, color: COLORS.cream, marginBottom: 6 }}>
                  "{v.line}"
                </div>
                <div style={{ fontFamily: TYPE.body, fontSize: 12, color: COLORS.plum }}>
                  {v.song} · {v.artist}
                </div>
              </div>
            ))}
